---
title: AlloyTeanCodeGuide
date: 2017-01-18 16:20:10
tags:
	- javascript
	- 前端
---

> [AlloyTeam CodeGuide](https://alloyteam.github.io/CodeGuide/) 前端编码规范
...
* EditorConfig [配置文件](https://alloyteam.github.io/CodeGuide/.editorconfig)
* JSCS [配置文件](https://alloyteam.github.io/CodeGuide/.jscsrc)
* JSHint [配置文件](https://alloyteam.github.io/CodeGuide/.jshintrc)
...
```
* CSSLint [配置文件](https://alloyteam.github.io/CodeGuide/.csslintrc)
* SCSS-Lint [配置文件](https://alloyteam.github.io/CodeGuide/.scss-lint.yml)
...
Preferences->Package Settings->JSFormat->Setting-User，下载[配置文件](https://alloyteam.github.io/CodeGuide/jsformat_setting_user.json)覆盖
...
Preferences->Package Settings->CSScomb->Setting-User，下载[配置文件](https://alloyteam.github.io/CodeGuide/csscomb_setting_user.json)覆盖
...
## 前端工程化或者持续集成(CI)方式使用Linter工具

### 什么是JSLint和JSHint?

JSLint是一个静态代码分析工具，JSHint 是在 JSLint 代码基础上二次开发而来的。
...
通过主流编辑器，IDE都有了JSHint的插件。外还有很多基于JSHint的[酷炫小项目](http://jshint.com/install/)

2.通过（工程化）前端构建工具基础

前端构建工具Grunt、Gulp和Webpack等都有其对应插件
通过这种方式集成进去会方便美观很多。

这里默认npm的相关知识你都知道，不清楚的还是先去了解一下。
...
然后，只需要运行`gulp hint`即可检查代码。
就是这么简单。

**JSHint的自定义规则**

通过在根目录创建文件`.jshintrc`，例如
```json
{
  "sub":true,
  "laxbreak":true,
...
```
具体的含义请参考[文档](http://jshint.com/docs/options/)。你也可以在你的项目目录下添加一个.jshintrc文件，JSHint会在检测文件时从文件所在目录往上找，直到找到jshintrc文件为止。

### ESLint

JSHint 不支持 JSX 语法。所以有了 ESLint 。
...
具体可查看[官方文档](http://eslint.org/docs/user-guide/integrations)
...
有时我们希望一些简单的错误可以工具帮助我们自动修复。

我们可以寻找一下方式，假如使用JSLint或JSHint,我们可以寻找一些编辑器插件。或者npm工具，如[fixmyjs](https://github.com/jshint/fixmyjs) 、 [gulp-fixmyjs](https://github.com/kirjs/gulp-fixmyjs)。

假如使用 eslint ，其自带参数 `-- --fix` 可帮助自动修复部分错误。
